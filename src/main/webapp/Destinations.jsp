...
String tempTime = "2022-06-06 00:00:00";
...
if(request.getParameter("save_all") != null){
	...
	for(int j = 0; j<station_toAr.length; j++){
		...
		tempDesti.bus_id= request.getParameter("dst_bus");
...
if(request.getParameter("search") != null){
	...
	allDestinaions = desti.getAll(request.getParameter("station_from"),request.getParameter("dst_bus"));
}
...
<% } %>
			...
			<div class="col-xs-12 col-sm-6 offset-sm-3">
				<div class="rs_form_box">
					...
					<div class="input-group">
						...
						<select name="dst_bus" class="form-control" style="width:auto;">
							<%
								while(busIt.hasNext()){
									Bus bussTemp = (Bus) busIt.next();
									%>
									<option value="<%= bussTemp.id %>"><%= bussTemp.name+" ("+bussTemp.code+")" %></option>
									<%
								}
							%>
						</select>
	...
	<% }else{ %>
		<div class="rs_box" style="overflow:auto;">
			...
			Station tempFromStation = new Station();
			tempFromStation = temptoFromStation.getStation(request.getParameter("station_from"));
			buses tempBus=new buses(request.getParameter("dst_bus"));
		
			%>
				<h2 class="title"> Bus Name: <%= tempBus.name %> <br>Station From: <%= tempFromStation.name %> </h2>
